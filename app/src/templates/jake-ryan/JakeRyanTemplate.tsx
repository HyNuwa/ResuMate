...
export const JakeRyanTemplate = memo(function JakeRyanTemplate({ resume }: JakeRyanTemplateProps) {
  const { basics, sections } = resume;

  const headingVariant = 'uppercase-border-bottom';
    ...
    <div style={baseTextStyle}>
      ...
      {basics.name && (
        <div style={{ marginBottom: '4pt' }}>
          <span style={headingStyle}>{basics.name}</span>
        </div>
      )}
      ...
      <ContactInfo
        ...
        variant="centered"
        showIcons={false}
        style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: '8pt', fontSize: '11px', color: '#6b7280', textAlign: 'center', marginBottom: '8pt' }}
      />
